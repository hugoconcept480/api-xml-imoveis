...
exports.handler = async function(event, context) {
    ...
    let urlFormat = params.url_format || null;
    ...
    if (rawString.includes('-')) {
        ...
        for (let i = 1; i < parts.length; i++) {
            ...
            // Tag FMT (Formato Customizado codificado em Base64)
            if (part === 'fmt' && parts[i+1]) {
                try {
                    // Decodifica de Base64 Url Safe para Texto normal
                    const encoded = parts[i+1];
                    const buff = Buffer.from(encoded.replace(/-/g, '+').replace(/_/g, '/'), 'base64');
                    urlFormat = buff.toString('utf-8');
                } catch (e) {
                    console.log("Erro decodificando formato:", e);
                }
                i++;
            }
        }
    ...
    try {
        ...
        const buildParams = { url_format: urlFormat, phone: clientPhone }; 
